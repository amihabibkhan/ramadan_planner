package info.ruhulamin.ramadanplanner.Model;

public class ReportModel {
    public int check_list_1, check_list_2, check_list_3, check_list_4, check_list_5, check_list_6, check_list_7, check_list_8, check_list_9,
            fazar_f, fazar_s, zohor_f, zohor_s, asor_f, asor_s, magrib_f, magrib_s, isha_f, isha_s, tarabih, tahazzud;
    public String tilawat_ayah, tilawat_sura, memorize_ayah, memorize_sura, self_criticism, achievement;

    public ReportModel() {
    }

    public ReportModel(int check_list_1, int check_list_2, int check_list_3, int check_list_4, int check_list_5, int check_list_6, int check_list_7, int check_list_8, int check_list_9, int fazar_f, int fazar_s, int zohor_f, int zohor_s, int asor_f, int asor_s, int magrib_f, int magrib_s, int isha_f, int isha_s, int tarabih, int tahazzud, String tilawat_ayah, String tilawat_sura, String memorize_ayah, String memorize_sura, String self_criticism, String achievement) {
        this.check_list_1 = check_list_1;
        this.check_list_2 = check_list_2;
        this.check_list_3 = check_list_3;
        this.check_list_4 = check_list_4;
        this.check_list_5 = check_list_5;
        this.check_list_6 = check_list_6;
        this.check_list_7 = check_list_7;
        this.check_list_8 = check_list_8;
        this.check_list_9 = check_list_9;
        this.fazar_f = fazar_f;
        this.fazar_s = fazar_s;
        this.zohor_f = zohor_f;
        this.zohor_s = zohor_s;
        this.asor_f = asor_f;
        this.asor_s = asor_s;
        this.magrib_f = magrib_f;
        this.magrib_s = magrib_s;
        this.isha_f = isha_f;
        this.isha_s = isha_s;
        this.tarabih = tarabih;
        this.tahazzud = tahazzud;
        this.tilawat_ayah = tilawat_ayah;
        this.tilawat_sura = tilawat_sura;
        this.memorize_ayah = memorize_ayah;
        this.memorize_sura = memorize_sura;
        this.self_criticism = self_criticism;
        this.achievement = achievement;
    }

    public int getCheck_list_1() {
        return check_list_1;
    }

    public void setCheck_list_1(int check_list_1) {
        this.check_list_1 = check_list_1;
    }

    public int getCheck_list_2() {
        return check_list_2;
    }

    public void setCheck_list_2(int check_list_2) {
        this.check_list_2 = check_list_2;
    }

    public int getCheck_list_3() {
        return check_list_3;
    }

    public void setCheck_list_3(int check_list_3) {
        this.check_list_3 = check_list_3;
    }

    public int getCheck_list_4() {
        return check_list_4;
    }

    public void setCheck_list_4(int check_list_4) {
        this.check_list_4 = check_list_4;
    }

    public int getCheck_list_5() {
        return check_list_5;
    }

    public void setCheck_list_5(int check_list_5) {
        this.check_list_5 = check_list_5;
    }

    public int getCheck_list_6() {
        return check_list_6;
    }

    public void setCheck_list_6(int check_list_6) {
        this.check_list_6 = check_list_6;
    }

    public int getCheck_list_7() {
        return check_list_7;
    }

    public void setCheck_list_7(int check_list_7) {
        this.check_list_7 = check_list_7;
    }

    public int getCheck_list_8() {
        return check_list_8;
    }

    public void setCheck_list_8(int check_list_8) {
        this.check_list_8 = check_list_8;
    }

    public int getCheck_list_9() {
        return check_list_9;
    }

    public void setCheck_list_9(int check_list_9) {
        this.check_list_9 = check_list_9;
    }

    public int getFazar_f() {
        return fazar_f;
    }

    public void setFazar_f(int fazar_f) {
        this.fazar_f = fazar_f;
    }

    public int getFazar_s() {
        return fazar_s;
    }

    public void setFazar_s(int fazar_s) {
        this.fazar_s = fazar_s;
    }

    public int getZohor_f() {
        return zohor_f;
    }

    public void setZohor_f(int zohor_f) {
        this.zohor_f = zohor_f;
    }

    public int getZohor_s() {
        return zohor_s;
    }

    public void setZohor_s(int zohor_s) {
        this.zohor_s = zohor_s;
    }

    public int getAsor_f() {
        return asor_f;
    }

    public void setAsor_f(int asor_f) {
        this.asor_f = asor_f;
    }

    public int getAsor_s() {
        return asor_s;
    }

    public void setAsor_s(int asor_s) {
        this.asor_s = asor_s;
    }

    public int getMagrib_f() {
        return magrib_f;
    }

    public void setMagrib_f(int magrib_f) {
        this.magrib_f = magrib_f;
    }

    public int getMagrib_s() {
        return magrib_s;
    }

    public void setMagrib_s(int magrib_s) {
        this.magrib_s = magrib_s;
    }

    public int getIsha_f() {
        return isha_f;
    }

    public void setIsha_f(int isha_f) {
        this.isha_f = isha_f;
    }

    public int getIsha_s() {
        return isha_s;
    }

    public void setIsha_s(int isha_s) {
        this.isha_s = isha_s;
    }

    public int getTarabih() {
        return tarabih;
    }

    public void setTarabih(int tarabih) {
        this.tarabih = tarabih;
    }

    public int getTahazzud() {
        return tahazzud;
    }

    public void setTahazzud(int tahazzud) {
        this.tahazzud = tahazzud;
    }

    public String getTilawat_ayah() {
        return tilawat_ayah;
    }

    public void setTilawat_ayah(String tilawat_ayah) {
        this.tilawat_ayah = tilawat_ayah;
    }

    public String getTilawat_sura() {
        return tilawat_sura;
    }

    public void setTilawat_sura(String tilawat_sura) {
        this.tilawat_sura = tilawat_sura;
    }

    public String getMemorize_ayah() {
        return memorize_ayah;
    }

    public void setMemorize_ayah(String memorize_ayah) {
        this.memorize_ayah = memorize_ayah;
    }

    public String getMemorize_sura() {
        return memorize_sura;
    }

    public void setMemorize_sura(String memorize_sura) {
        this.memorize_sura = memorize_sura;
    }

    public String getSelf_criticism() {
        return self_criticism;
    }

    public void setSelf_criticism(String self_criticism) {
        this.self_criticism = self_criticism;
    }

    public String getAchievement() {
        return achievement;
    }

    public void setAchievement(String achievement) {
        this.achievement = achievement;
    }
}
